...
val AGENT_DEFINITION = """
    ...
    through `await llm(systemMessage: string, userMessage: string, ...args: any[]): string` function call.
    ...
    should be provided with a key that was used in `put` function.
    ...
    ```js
        ...
        function read_files(files) {
            for (const file of files) {
                localMemory.put(summarize(file));
            }
        }
